...
rem ====================
rem
rem Portions Copyrighted 2012-2025 Ping Identity Corporation
rem
rem -- END LICENSE
...
set JAVA_DLL="%JAVA_HOME%\bin\server\jvm.dll"
:homeOk
...
set JAVA_OPTS=-Xmx1g -Xms1g

rem Service java options, needs to be separated by ;
set JAVA_OPTS_SERVICE=-Xmx1g;-Xms1g;
rem Enable SSL 
...
:srvSetKey
rem Sets key in the Java Remote Connectors Server properties file
if not ""%2"" == """" (
...
:srvInstall
rem Install the Java Remote Connector Server as Windows service
shift
...
"%CONNECTOR_SERVER_HOME%\bin\%ARCH%\ConnectorServerJava.exe" //IS//%SERVICE_NAME% --Install="%CONNECTOR_SERVER_HOME%\bin\%ARCH%\ConnectorServerJava.exe" --DisplayName="ForgeRock Java Remote Connector Server" --Description="ForgeRock Java Remote Connector Server" --Startup=auto --Jvm=%JAVA_DLL% --Classpath=%CP% --JvmOptions=%JAVA_OPTS_SERVICE%%JAVA_OPTS_PARAM% --StartPath="%CONNECTOR_SERVER_HOME%" --StartMode=jvm --StartClass=%MAIN_CLASS% --StartParams="-service;-properties;%SERVER_PROPERTIES%" --StopMode=jvm --StopClass=%MAIN_CLASS% --StopMethod=stop --StopParams=dummy --LogPath="%CONNECTOR_SERVER_HOME%\logs" --LogPrefix=service --StdOutput=auto --StdError=auto --LogLevel=INFO
echo ForgeRock Java Remote Connector Server successfully installed as "%SERVICE_NAME%" service
goto :EOF
